...
style.textContent = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ft-screen.embedded { padding: 16px 20px 100px; min-height: unset; }
...
function HomeScreen({ workouts }) {
  ...
  return (
    <div className={`ft-screen ft-slide-up ${embedded?"embedded":""}`}>
...
function RoutinesScreen({ onStartRoutine, onStartProgramWorkout, embedded }) {
  const [view, setView] = useState("routines"); // "routines" | "library" | "programs"
    ...
    <div className={`ft-screen ft-slide-up ${embedded?"embedded":""}`}>
      ...
      <div className="view-toggle" style={{ marginBottom: 16 }}>
        ...
        <button className={`view-toggle-btn ${view==="library"?"active":""}`} onClick={()=>setView("library")} style={{ fontFamily:"'DM Sans'" }}>
          📚 Exercises ({Object.keys(TUTORIALS).length})
        </button>
        <button className={`view-toggle-btn ${view==="programs"?"active":""}`} onClick={()=>setView("programs")} style={{ fontFamily:"'DM Sans'" }}>
          🫁 12-Week
        </button>
      </div>

      {view === "library" ? <ExerciseLibraryView /> :
       view === "programs" ? <ProgramsScreen onStartWorkout={onStartProgramWorkout}/> :
       (
        <>
          {/* Category filter */}
...
function LogScreen({ onSave, prefill, embedded }) {
  ...
  if (saved) return (
    <div className={`ft-screen ${embedded?"embedded":""}`} style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center" }}>
  ...
  return (
    <div className={`ft-screen ft-slide-up ${embedded?"embedded":""}`} style={{ paddingBottom:120 }}>
...
function ProgressScreen({ workouts, embedded }) {
...
function CoachScreen({ workouts, embedded }) {
  ...
  return (
    <div className={`ft-screen ft-slide-up ${embedded?"embedded":""}`} style={{ paddingBottom:160 }}>
...
function BendSession({ program, durationKey, onComplete, onExit }) {
  ...
  // ── Done screen
  if (done) return (
    <div className={`ft-screen ${embedded?"embedded":""}`} style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",background:"#0a0a0f" }}>
  ...
  if (!started) return (
    <div className={`ft-screen ft-slide-up ${embedded?"embedded":""}`} style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center" }}>
...
function CardioScreen({ onSave }) {
  ...
  if (step === "done") {
    ...
    return (
      <div className={`ft-screen ft-slide-up ${embedded?"embedded":""}`} style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", gap:12 }}>
...
// ── Cable Main Screen ─────────────────────────────────────────────────────────
function CableScreen({ onLogWorkout, embedded }) {
  const [cat, setCat] = useState("All");
  const [selected, setSelected] = useState(null);
...
function SchedulerScreen({ workouts, onNavigateToLog, routinesAll, embedded }) {
  ...
  return (
    <div className={`ft-screen ft-slide-up ${embedded?"embedded":""}`} style={{ paddingBottom:100 }}>
...
function RecoverySession({ protocol, onDone }) {
  ...
  if (done) return (
    <div className={`ft-screen ${embedded?"embedded":""}`} style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",gap:16 }}>
      <div style={{ fontSize:72 }}>✨</div>
...
Rules: supersets get superset:true on the FIRST of the pair only, with supersetWith naming the next exercise. For ≤30 min sessions use supersets/circuits. Only use equipment available. Match exercises to experience level. For golf goals include rotational and stability work.`;
  ...
  if (phase==="loading") return (
    <div className={`ft-screen ${embedded?"embedded":""}`} style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:24,textAlign:"center"}}>
  ...
  if (phase==="nokey") return (
    <div className={`ft-screen ${embedded?"embedded":""}`} style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,textAlign:"center"}}>
...
// ═══════════════════════════════════════════════════════════════════════════════
// NAV HUB COMPONENTS  (merge screens behind a top toggle to reduce nav items)
// ═══════════════════════════════════════════════════════════════════════════════

// Shared toggle bar used by all hubs
function HubToggle({ options, active, onChange }) {
  return (
    <div style={{
      display:"flex", background:"#13131e", borderRadius:12, padding:3,
      ...
      {options.map(o => (
        <button key={o.id} onClick={() => onChange(o.id)}
          style={{
            ...
            fontFamily:"'DM Sans'", fontSize:12, fontWeight:600, transition:"all .2s",
            background: active===o.id ? "#c8f135" : "transparent",
            color: active===o.id ? "#0a0a0f" : "#555570",
          }}>
          {o.label}
        </button>
      ))}
...
// ── Recover Hub: Stretch + Recovery ─────────────────────────────────────────
function RecoverHub() {
  const [view, setView] = useState("stretch");
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:"#0a0a0f" }}>
      <div style={{ padding:"56px 20px 10px" }}>
        <HubToggle
          options={[{id:"stretch",label:"🧘 Stretch"},{id:"recovery",label:"🌊 Recovery"}]}
          active={view} onChange={setView}
        />
      </div>
      <div style={{ flex:1, overflowY:"auto" }}>
        {view==="stretch"  ? <StretchesScreen  embedded/> : <RecoveryScreen embedded/>}
      </div>
    </div>
  );
}

// ── Stats Hub: Progress + Schedule ─────────────────────────────────────────
function StatsHub({ workouts, onNavigateToLog, routinesAll }) {
  const [view, setView] = useState("stats");
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:"#0a0a0f" }}>
      <div style={{ padding:"56px 20px 10px" }}>
        <HubToggle
          options={[{id:"stats",label:"📊 Stats"},{id:"schedule",label:"📅 Schedule"}]}
          active={view} onChange={setView}
        />
      </div>
      <div style={{ flex:1, overflowY:"auto" }}>
        {view==="stats"
          ? <ProgressScreen workouts={workouts} embedded/>
          : <SchedulerScreen workouts={workouts} onNavigateToLog={onNavigateToLog} routinesAll={routinesAll} embedded/>
        }
      </div>
    </div>
  );
...
// ── AI Hub: Coach + Settings ─────────────────────────────────────────────────
function AIHub({ workouts }) {
  const [view, setView] = useState("coach");
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:"#0a0a0f" }}>
      <div style={{ padding:"56px 20px 10px" }}>
        <HubToggle
          options={[{id:"coach",label:"🤖 AI Coach"},{id:"settings",label:"⚙️ API Keys"}]}
          active={view} onChange={setView}
        />
      </div>
      <div style={{ flex:1, overflowY:"auto" }}>
        {view==="coach" ? <CoachScreen workouts={workouts} embedded/> : <SettingsScreen embedded/>}
      </div>
    </div>
  );
}

// ── Train Hub: Library + Cable + Log ────────────────────────────────────────
function TrainHub({ workouts, onStartRoutine, onStartProgramWorkout, onLogWorkout, prefill, onSave }) {
  const [view, setView] = useState("library");

  // Auto-switch to Log when a routine/program is started (prefill arrives)
  useEffect(() => {
    if (prefill) setView("log");
  }, [prefill]);
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:"#0a0a0f" }}>
      <div style={{ padding:"56px 20px 10px" }}>
        <HubToggle
          ...
          active={view} onChange={v => { if(v==="log"&&!prefill) {} setView(v); }}
        />
      </div>
      <div style={{ flex:1, overflowY:"auto" }}>
        {view==="library" && <RoutinesScreen onStartRoutine={onStartRoutine} onStartProgramWorkout={onStartProgramWorkout} embedded/>}
        {view==="cable"   && <CableScreen onLogWorkout={onLogWorkout} embedded/>}
        {view==="log"     && <LogScreen onSave={onSave} prefill={prefill} embedded/>}
      </div>
    </div>
  );
}
...
export default function FitTrack() {
  ...
  const navItems = [
    ...
    { id:"train",   label:"Train",   icon:Icon.routines },
    { id:"cardio",  label:"Cardio",  icon:Icon.cardio },
    { id:"golf",    label:"Golf",    icon:Icon.golf },
    { id:"recover", label:"Recover", icon:Icon.recovery },
    { id:"build",   label:"Build",   icon:Icon.generate },
    { id:"stats",   label:"Stats",   icon:Icon.stats },
    { id:"ai",      label:"AI",      icon:Icon.bot },
  ];

  return (
    <div className="ft-root">
      {tab==="home"    && <HomeScreen workouts={workouts}/>}
      {tab==="train"   && <TrainHub workouts={workouts}
                            onStartRoutine={handleStartRoutine}
                            onStartProgramWorkout={handleStartProgramWorkout}
                            onLogWorkout={handleCableLog}
      ...
      {tab==="build"   && <GeneratorScreen workouts={workouts} onSaveWorkout={saveWorkout}/>}
      {tab==="stats"   && <StatsHub workouts={workouts} onNavigateToLog={()=>{setRoutinePrefill(null);setTab("train");}} routinesAll={ROUTINES_ALL}/>}
      {tab==="ai"      && <AIHub workouts={workouts}/>}
      ...
      <nav className="ft-nav">
        {navItems.map(n => (
          <button key={n.id} className={`ft-nav-btn ${tab===n.id?"active":""}`}
            onClick={()=>{ if(n.id==="train"&&routinePrefill) {} setTab(n.id); }}>
            {n.icon}{n.label}
          </button>
        ))}
      </nav>
